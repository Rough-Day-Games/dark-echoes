...
public record MobProgression(String target, int actions, String pendingTarget, int pendingActions, int slots) {
    public static final MobProgression EMPTY = new MobProgression("", 0, "", 0, 0);
    public static final Codec<MobProgression> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codec.intRange(0, Integer.MAX_VALUE).optionalFieldOf("slots", 0).forGetter(MobProgression::slots)
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, MobProgression> STREAM_CODEC = StreamCodec.composite(
            ...
            ByteBufCodecs.VAR_INT, MobProgression::slots,
            MobProgression::new);
    ...
    public MobProgression advance(String mob, int actionsPerLevel, int maxLevel, int currentSlot) {
        int maximumActions = actionsPerLevel * maxLevel;
        if (locked()) {
            if (!target.equals(mob) || actions >= maximumActions) {
                return this;
            }
            return new MobProgression(target, actions + 1, "", 0, currentSlot);
        }

        int nextPending = pendingTarget.equals(mob) ? pendingActions + 1 : 1;
        if (nextPending >= actionsPerLevel) {
            return new MobProgression(mob, actionsPerLevel, "", 0, currentSlot);
        }
        return new MobProgression("", 0, mob, nextPending, currentSlot);
    }
}
